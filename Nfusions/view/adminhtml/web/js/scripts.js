    ...
    AdminOrder.prototype = {
        ...
        productGridRowClick : function(grid, event){
            ...
            if (trElement && !isInputQty) {
                ...
                var confLink = eventElement;
                var priceColl = Element.select(trElement, '.price')[0];
                if (checkbox) {
                    // processing non composite product
					 var elemClassName = confLink.getAttribute("class");
					   if ((" " + elemClassName + " ").replace(/[\n\t]/g, " ").indexOf('action-tier') > -1) {
							var hasclas = true;
					   }
					   else if ((" " + elemClassName + " ").replace(/[\n\t]/g, " ").indexOf('action-configure') > -1) {
							var hasclas = true;
					   }
					   else{
						   var hasclas = false;
					   }
                    if (confLink.readAttribute('disabled') || !hasclas ) {
